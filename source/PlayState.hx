...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'glow-up':
				dialogue = CoolUtil.coolTextFile(Paths.txt('glow-up/glowDialogue'));
			case 'moonlight':
				dialogue = CoolUtil.coolTextFile(Paths.txt('moonlight/moonDialogue'));
			case 'omnipresence':
				dialogue = CoolUtil.coolTextFile(Paths.txt('omnipresence/omniDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'stage':
				...
				case 'neocity':
				{
						defaultCamZoom = 0.9;
						curStage = 'neocity';
						bg1 = new FlxSprite(-600, -200).loadGraphic(Paths.image('neocity'));
						bg1.antialiasing = true;
						bg1.scrollFactor.set(0.9, 0.9);
						bg1.active = false;
						add(bg1);

						bg2 = new FlxSprite(-600, -200).loadGraphic(Paths.image('mbg'));
...
var bg1:FlxSprite;
var bg2:FlxSprite;
var didBgSwitch:Bool = false;
var didBgSwitchBack:Bool = false;
	...
	if(!didBgSwitch && SONG.song == "omnipresence" && Conductor.songPosition >= (1*60 + 13)*1000) {
		didBgSwitch = true;
  FlxTween.tween(bg2, {alpha:1}, 3 /* seconds */);

}

if(!didBgSwitchBack && SONG.song == "omnipresence" && Conductor.songPosition >= (1*60 + 31)*1000) {
		didBgSwitchBack = true;
  FlxTween.tween(bg2, {alpha:0}, 3 /* seconds */);
